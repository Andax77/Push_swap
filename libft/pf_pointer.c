...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/02/15 16:19:05 by anhuang           #+#    #+#             */
/*   Updated: 2018/02/15 16:19:06 by anhuang          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

static void	pf_type_pointer4(long long *nbr, t_params para)
{
	if (para.type == 'h')
...
static void	pf_pupointer(long nbr, t_params *pa)
{
	int lol;
	...
	|| (pa->flag.zero && pa->width > 0)))
		pf_pupointer(nbr / (unsigned int)pa->flag.base, pa);
	else
	{
...
void		pf_pointer(va_list p, t_params pa)
{
	...
	if (nbr == 0 && pa.precision == 0)
		pc_putstr("0x");
	pf_type_pointer4(&nbr, pa);
	if (pa.flag.hash)
		pa.width -= 2;
