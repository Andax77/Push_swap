...
void		pf_type_big_octal4(long long *nbr, t_params para)
{
	if (para.type == 'h')
...
void		pf_big_octal(va_list p, t_params para)
{
	para.type = 'l';
	if (para.type)
		pf_big_octal2(p, para);
	else
		pf_big_octal1(p, para);
}

static void	pf_big_puoctal(long nbr, t_params *pa)
{
	int	lol;
	...
	|| (pa->flag.zero && pa->width > 0)))
		pf_big_puoctal(nbr / (unsigned int)pa->flag.base, pa);
	else
	{
